...
- 캡처 시 고품질 후처리: 색감 틴트 · 블룸 · 그레인 · 비네팅 · 스캔라인 · 날짜각인 · 폴라로이드 테두리
- 셔터음(WebAudio 합성) · 진동(Vibration API) · 현상 애니메이션
- 전/후면 전환, 디지털 줌, 날짜/사운드 토글
- 필름롤(IndexedDB 로컬 저장) · 저장/공유(Web Share → iOS 사진앱 저장)
- 오프라인 동작(서비스워커), PWA 설치
